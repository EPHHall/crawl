	...
	bool print_unseen, bool display)
{
	...
	for (const god_type god : gods)
	{
		...
		if (colour == "blue")
			symbol = " (bl)";
		string disp_name = uppercase_first(god_name(god, false)) + symbol;
		...
		if (num_printed % columns == 0)
			...
			{
			case 1:
				disp += string(19 - strwidth(disp_name), ' ');
				break;
			case 2:
				disp += string(23 - strwidth(disp_name), ' ');
				break;
			case 3:
				disp += string(20 - strwidth(disp_name), ' ');
				break;
			}
	}
...
static void _process_command(const char keypress)
{
	dgn_overview colorKey
	(

		"Color Codes:\n"

		"<darkgrey>Dark Grey (dg): Altar has not been found OR the god is unavailable</darkgrey>\n"

		"<white>White (w): Altar has been found</white>\n"

		"<lightred>Light Red (lr): You owe penance to this god and may be punished</lightred>\n"

		"<red>Red (r): You owe a lot of penance to this god and may be punished</red>\n"

		"<magenta>Magenta (m): You owe penance to this god, but will not be punished (usually because the god is good)</magenta>\n"

		"<yellow>Yellow (y): You worship this god</yellow>\n"

		"<brown>Brown (br): You have found an altar to a god your god likes</brown>\n"

		"<blue>Blue (bl): Altar is seen but not reachable</blue>\n"
	);
	switch (keypress)
	{
	case 'G':
